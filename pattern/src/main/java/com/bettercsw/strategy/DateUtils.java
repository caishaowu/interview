...
public class DateUtils {
    ...
    public static String getFuDate(int month) {
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
        Calendar cal = Calendar.getInstance();
        cal.add(Calendar.MONTH, month);//对月份进行计算,减去12个月
        Date date = cal.getTime();
     ...
     * @return
     */
    public static String getCurrentTime1(final String parsePatterns) {
        Date date = new Date();
    ...
    public static LocalDate getEndDayOfMonth(LocalDate date) {
        ...
        return LocalDate.of(date.getYear(), month, length);
    }
    ...
    public static LocalDate getEndDayOfQuarter(LocalDate date) {
        ...
        Month endMonOfQuarter = Month.of(firstMonthOfQuarter.getValue() + 2);
        return LocalDate.of(date.getYear(), endMonOfQuarter, endMonOfQuarter.length(date.isLeapYear()));
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(getCurrentTime1(YYYY_MM_DD_HHmmss));

        try {
            System.out.println(getDateStr("1998/10/02 00:00:00", YYYYMMDD_HHmmss));
        } catch (Exception e) {
            e.printStackTrace();
        }

        System.out.println("fudate:" + getFuDate(1));
